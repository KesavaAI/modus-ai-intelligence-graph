...
export function CascadeImpactDrawer({
  ...
  useEffect(() => {
    ...
    async function fetchCascade() {
      ...
      if (apiBase) {
        try {
          const res = await fetch(`${apiBase}/api/v1/intelligence/cascade`, {
            method: "POST",
          ...
          console.warn("Falling back to client-side cascade computation");
        }
      }

      // Standalone client calculation fallback
      const nodes = defaultGraphData.nodes as any[];
